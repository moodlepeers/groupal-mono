...
{
    static class TwittleCombinatoryCalculator
    {
...
*/
    ...
    static public int twiddle(ref int x, ref int y, ref int z, ref int[] p)    
    {
    int i, j, k;
    j = 1;
    while(p[j] <= 0)
...
static public void inittwiddle(int m, int n, ref int[]p)
  {
  int i;
  p[0] = n+1;
  for(i = 1; i != n-m+1; i++)
